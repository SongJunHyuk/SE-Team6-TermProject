...
<%
	...
	String sql = "SELECT oid FROM Customer WHERE phoneNumber=?"; //전화번호로 고객id 가져온다
	...
	<%while(rs.next()){//예약이 존재한다면 출력
		...
		<div class="text-center">
			예약번호: <%=rs.getString("oid") %> 예약날짜: <%=rs.getString("date") %>  예약인원: <%=rs.getString("covers") %> 예약시간: <%=rs.getString("time") %><br>
			예약 테이블: <%=rs.getString("table_id") %> 이벤트: <%=rs.getString("event") %><br>
